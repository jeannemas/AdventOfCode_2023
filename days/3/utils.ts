...
export function getAdjacentCells(partNumber: PartNumber, schematic: Schematic) {
  ...
  const topLeftCell = hasCellsOnTop && hasCellsOnLeft
    ? ({ x: partNumber.x - 1, y: partNumber.y - 1 } satisfies Cell)
    : null;
  const topRightCell = hasCellsOnTop && hasCellsOnRight
    ? ({
      x: partNumber.x + partNumber.span,
      y: partNumber.y - 1,
    } satisfies Cell)
    : null;
  const bottomLeftCell = hasCellsOnBottom && hasCellsOnLeft
    ? ({ x: partNumber.x - 1, y: partNumber.y + 1 } satisfies Cell)
    : null;
  const bottomRightCell = hasCellsOnBottom && hasCellsOnRight
    ? ({
      x: partNumber.x + partNumber.span,
      y: partNumber.y + 1,
    } satisfies Cell)
...
export function getPartNumber(cell: Cell, schematic: Schematic) {
  ...
  while (
    partNumber.x - 1 >= 0 &&
    /\d/.test(schematic[partNumber.y][partNumber.x - 1])
  ) {
    partNumber.x -= 1;
  }
...
export function getPartNumberValue(
  partNumber: PartNumber,
  schematic: Schematic,
) {
  return +schematic[partNumber.y].substring(
    partNumber.x,
    partNumber.x + partNumber.span,
  );
}
